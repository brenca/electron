...
void OffscreenWindow::OnRendererResponsive(content::RenderProcessHost*) {
  window_unresponsive_closure_.Cancel();
  Emit("responsive");
}
...
void OffscreenWindow::SetOwnerWindow(v8::Local<v8::Value> value) {
  gin::Handle<BrowserWindow> browser_window;
  if (value->IsObject() &&
...
v8::Local<v8::Value> OffscreenWindow::GetWebContentsView(v8::Isolate* isolate) {
  if (web_contents_view_.IsEmpty())
    return v8::Null(isolate);
  return v8::Local<v8::Value>::New(isolate, web_contents_view_);
}
...
// static
gin_helper::WrappableBase* OffscreenWindow::New(
  ...
  if (!(args->Length() == 1 && args->GetNext(&options))) {
    options = gin_helper::Dictionary::CreateEmpty(args->isolate());
  }
...
void OffscreenWindow::BuildPrototype(
  ...
  prototype->SetClassName(gin::StringToV8(isolate, "OffscreenWindow"));
  ...
  gin_helper::ObjectTemplateBuilder(isolate, prototype->PrototypeTemplate())
      ...
      .SetProperty("webContentsView", &OffscreenWindow::GetWebContentsView)
...
NODE_LINKED_MODULE_CONTEXT_AWARE(electron_browser_offscreen_window, Initialize)
